...
export default async function AdminDashboard() {
  ...
  return <AdminShell active="dashboard">
    <div className="adminTitle"><div><span>OVERVIEW</span><h1>Dashboard</h1><p>Live overview of Cocktaillo orders and menu activity.</p></div><a className="adminPrimary" href="/admin/orders">Manage orders</a></div>
    <div className="statsGrid">
      <Stat icon={<ClipboardList/>} label="Total orders" value={String(totalOrders)}/>
      <Stat icon={<Clock3/>} label="Active orders" value={String(openOrders)}/>
      <Stat icon={<ShoppingBag/>} label="Menu items" value={String(products)}/>
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="statCard"><div className="statIcon">{icon}</div><div><small>{label}</small><strong>{value}</strong></div></div>
}
