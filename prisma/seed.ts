...
const defaults: Record<string, string> = {
  ...
  phone: '', whatsapp: '', whishEnabled: 'true', whishPhone: '', cashEnabled: 'true',
  deliveryEnabled: 'true', takeawayEnabled: 'true', dineInEnabled: 'true', deliveryFee: '0',
  instagram: '', facebook: '', tiktok: '', locationUrl: '', menuSourceUrl
}
...
async function seedImportedMenu() {
  ...
  for (const item of items) {
    ...
    if (!categoryId) {
      const category = await db.category.create({ data: { name: cat, slug: `${slugify(cat)}-${categoryIds.size + 1}` } })
      categoryId = category.id
      categoryIds.set(cat, categoryId)
    }
    await db.product.create({ data: { name: item.name, description: item.description || null, price: item.price, imageUrl: item.imageUrl || null, categoryId } })
  }
...
async function main() {
  for (const [key, value] of Object.entries(defaults)) await db.setting.upsert({ where: { key }, update: {}, create: { key, value } })
