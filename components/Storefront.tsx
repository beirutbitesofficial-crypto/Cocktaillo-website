...
import { Instagram, Facebook, MapPin, Minus, Plus, Search, ShoppingBag, X, CheckCircle2, UtensilsCrossed, Bike, PackageCheck, CreditCard, Banknote, Phone } from 'lucide-react'
...
export default function Storefront({ categories, settings }: Props) {
  ...
  const [tableFromUrl, setTableFromUrl] = useState('')
  ...
  useEffect(() => {
    ...
    setTableFromUrl(table)
    if (table) setOrderType('DINE_IN')
  }, [])
  useEffect(() => { localStorage.setItem('cocktaillo-cart', JSON.stringify(cart)) }, [cart])

  const products = useMemo(() => categories.flatMap(c => c.products.map(p => ({ ...p, categoryId: c.id, categoryName: c.name }))).filter(p => {
  ...
  async function checkout(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      tableNumber: String(fd.get('tableNumber') || ''),
    ...
    try {
      ...
      setSuccess({ orderNumber: data.orderNumber, total: data.total }); setCart([]); setCheckoutOpen(false); setCartOpen(false)
    } catch (err) { setError(err instanceof Error ? err.message : 'Could not place order') }
    finally { setSubmitting(false) }
  }

  const enabled = {
    DELIVERY: settings.deliveryEnabled === 'true', TAKEAWAY: settings.takeawayEnabled === 'true', DINE_IN: settings.dineInEnabled === 'true'
  }
  ...
  return <>
    ...
    <main id="top">
      <section className="hero">
        <div className="container heroGrid">
          <div className="heroCopy">
            ...
            <p>{settings.tagline || 'Order your Cocktaillo favorites for delivery, takeaway, or directly from your table.'}</p>
            ...
            <div className="servicePills">
              ...
              {enabled.DINE_IN && <span><UtensilsCrossed size={16}/> Dine-in</span>}
            </div>
          </div>
      ...
      <section className="orderStrip" id="order"><div className="container threeCols">
        ...
        <div><UtensilsCrossed/><div><strong>Dine In</strong><small>Scan, order & enjoy</small></div></div>
      </div></section>
    ...
    {cartOpen && <div className="drawerOverlay" onMouseDown={e => e.target === e.currentTarget && setCartOpen(false)}><aside className="cartDrawer"><div className="drawerHead"><div><small>YOUR ORDER</small><h2>Cart <span>({count})</span></h2></div><button onClick={() => setCartOpen(false)}><X/></button></div>
      <div className="cartItems">{cart.length ? cart.map(i => <div className="cartItem" key={i.id}><div className="cartThumb">{i.imageUrl ? <img src={i.imageUrl}/> : <span>C</span>}</div><div className="cartText"><strong>{i.name}</strong><small>{money(i.price)}</small><div className="qty"><button onClick={() => change(i.id,-1)}><Minus size={14}/></button><span>{i.quantity}</span><button onClick={() => change(i.id,1)}><Plus size={14}/></button></div></div><strong>{money(i.price*i.quantity)}</strong></div>) : <div className="emptyCart"><ShoppingBag/><h3>Your cart is empty</h3><p>Add something delicious from the menu.</p></div>}</div>
    ...
    {checkoutOpen && <div className="modalOverlay"><div className="checkoutModal"><div className="drawerHead"><div><small>CHECKOUT</small><h2>Complete your order</h2></div><button onClick={() => setCheckoutOpen(false)}><X/></button></div>
      <form onSubmit={checkout}>
        <div className="checkoutSection"><label className="fieldLabel">How would you like your order?</label><div className="optionGrid">
          ...
          {enabled.DINE_IN && <button type="button" className={orderType==='DINE_IN'?'selected':''} onClick={()=>setOrderType('DINE_IN')}><UtensilsCrossed/><span>Dine In</span></button>}
        </div></div>
        <div className="formGrid"><label>Name<input name="customerName" required placeholder="Your name"/></label>{orderType !== 'DINE_IN' && <label>Phone<input name="phone" required inputMode="tel" placeholder="03 123 456"/></label>}{orderType === 'DELIVERY' && <label className="full">Delivery address<textarea name="address" required placeholder="Area, street, building, floor..."/></label>}{orderType === 'DINE_IN' && <label>Table number<input name="tableNumber" required defaultValue={tableFromUrl} placeholder="e.g. 12"/></label>}<label className="full">Notes <span>(optional)</span><textarea name="notes" placeholder="Any special instructions?"/></label></div>
        <div className="checkoutSection"><label className="fieldLabel">Payment method</label><div className="paymentOptions">{settings.cashEnabled==='true' && <button type="button" className={payment==='CASH'?'selected':''} onClick={()=>setPayment('CASH')}><Banknote/><div><strong>Cash</strong><small>Pay when you receive your order</small></div></button>}{settings.whishEnabled==='true' && <button type="button" className={payment==='WHISH'?'selected':''} onClick={()=>setPayment('WHISH')}><CreditCard/><div><strong>Whish Money</strong><small>Transfer to {settings.whishPhone || 'restaurant Whish number'}</small></div></button>}</div>{payment==='WHISH' && <div className="whishBox"><strong>Send your payment via Whish</strong><p>Whish number: <b>{settings.whishPhone || 'Not configured yet'}</b></p><label>Transfer reference / sender name<input name="paymentReference" placeholder="Optional reference"/></label></div>}</div>
